...
function save_update_emp(){
    ...
    var department = document.getElementById("departmentName").value.split(",");
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            allEmps[index].description = descr;
            createTable();
    ...
    if(validSurname && validName){
        $.ajax({
            ...
            success : function(data) {
                ...
                console.log(data)
                allEmps.push({'id': Number(data), 'surname': surname, 'name': name, 'fatherName': fatherName,
    ...
    for (i=0; i<tableE.length;i++) {
        ...
        if(index === id){
            ...
            result+="<tr scope=\"row\"><form><td class=\"\" id=\"emp_id\">" + id + "</td>" +
                ...
                "<td class=\"\"><select id=\"departmentName\"><option selected value=\""+tableE[i].department.id+ "," +tableE[i].department.depName
                + ","+ tableE[i].department.description+"\">"+tableE[i].department.depName+"</option>";

            for (x=0; x<allDeps.length;x++) {
                if(Number(dep_id) !== allDeps[x].id){
                    result+="<option value=\""+tableE[i].department.id+ "," +tableE[i].department.depName
                    + ","+ tableE[i].department.description+"\">"+tableE[i].department.depName+"</option>";
                }
            }
